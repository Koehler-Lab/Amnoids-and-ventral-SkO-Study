# Extended Data Figure 7: Schwann cell depletion, size scalability, and transcriptional divergence in early epithelial organoids.
library(Seurat)
library(SeuratDisk)
library(ggplot2)
library(patchwork)
library(pbapply)
library(Nebulosa)
library(ComplexHeatmap)
source("rFunctions.R")


# load P307 annotated by P239.3
labels <- c("early", "mid", "late")
i <- 1
label <- labels[i]
# use UMAP transferred from Project 239.3 - used this
P307 <- readRDS(file = paste0("P307", label, "_P239.3Annotated.rds"))
Y <- P307
Yearly <- Y
Y2 <- Y


# decided to use col2 based on email from Phuong on 4/30/204
library(RColorBrewer)
col2 <- c(
  brewer.pal(9, "YlOrBr")[2:5],
  brewer.pal(9, "Oranges")[6],
  brewer.pal(7, "Reds")[-1],
  brewer.pal(9, "Blues"),
  brewer.pal(4, "GnBu")[3:4],
  brewer.pal(4, "Greens"),
  brewer.pal(5, "Purples"),
  brewer.pal(3, "PuRd")[3]
)
length(col2)
names(col2) <- levels(P239.3$CT)
matchColors2 <- col2

gg_color_hue <- function(n) {
  hues <- seq(15, 375, length = n + 1)
  hcl(h = hues, l = 65, c = 100)[1:n]
}


# Extended Figure 7e. UMAP visualization of day 6 SEO and cSkO single-cell transcriptomes reveals divergence in epithelial and neural crest cell populations.
png(paste0("P307", label, "_P239.3Annotated_SampleSplit.png"), width = 1500 * length(unique(Y$sample)), height = 1500, res = 300)
DimPlot(Y2, label = TRUE, label.size = 4, repel = TRUE, reduction = "umap", split.by = "sample", ncol = length(unique(Y$sample))) +
  theme_light() +
  theme(
    panel.border = element_blank(), panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    axis.text.x = element_text(size = 0), axis.text.y = element_text(size = 0),
    axis.ticks.x = element_blank(), axis.ticks.y = element_blank(),
    strip.background = element_rect(colour = NA, fill = NA),
    strip.text = element_text(color = "black", size = 16),
    legend.position = "None"
  ) +
  xlab("") +
  ylab("") +
  scale_color_manual(values = matchColors2[levels(Idents(Y2))])
dev.off()


# Extended Figure 7f. Heatmap of differentially expressed genes (DEGs) comparing day 6 SEO (±LDN/FGF) and cSkO, grouped by functional gene categories.
### Heatmap by sample
dge <- Y
X <- Y
library(reshape2)
geneslist <- list(
  "Extracellular matrix remodeling and cell migration" = c("CD44", "MMP9", "BGN"),
  "Ca²⁺, cAMP, and ERK signaling integrators" = c("TESC", "PPP1R1B", "KCNMB1", "ADRB2", "RCSD1", "TBX2"),
  "Neurotrophic and secretory" = c("SYT4", "SLITRK6", "STC1", "MAF", "G0S2"),
  "Extraembryonic keratins" = c("KRT7", "MFAP5"),
  "Endogenous BMP antagonists" = c("NOG", "FST"),
  "Transport and barrier functions" = c("SLC15A2", "SLC40A1", "CLIC3", "MUC15"),
  "Fluid-handling" = c("NPPB", "CA2"),
  "Mechanical stress-response factors" = c("ATF3", "MAFF", "IFI6"),
  "Key transcriptional regulators of amniotic epithelium" = c("HAND1", "GABRP", "GATA3", "SOX7")
)
genes <- unlist(geneslist)
print(genes[!(genes %in% rownames(dge))])
genes <- genes[which(genes %in% rownames(dge))]
genesgroup <- melt(geneslist)

library(ComplexHeatmap)

sample <- read.table("../../sample_name2.txt", sep = "\t", stringsAsFactors = F)
id <- as.character(X$orig.ident)
names(id) <- names(X$orig.ident)
id2 <- id
for (ii in 1:nrow(sample)) {
  s1 <- sample[ii, 1]
  s2 <- sample[ii, 3]
  id2[which(id == s1)] <- s2
}
table(id, id2)

X$sample2 <- factor(id2, levels = names(table(id2))[c(2, 3, 1)])
table(X$sample2)

Idents(X) <- X$sample2
centroid <- log(AverageExpression(X)$RNA + 1)
centroid.std <- (centroid - apply(centroid, 1, mean)) / apply(centroid, 1, sd)
# centroid.std=na.omit(centroid.std)
centroid.std[is.nan(centroid.std)] <- NA

genes[!(genes %in% rownames(centroid))]
genes[!(genes %in% rownames(centroid.std))]
# Lineage  "TBX1"
length(which(genes %in% rownames(centroid.std))) # 17
genes2 <- genes[(genes %in% rownames(centroid.std))]

cc <- centroid[which(rownames(centroid) %in% genes), ]
cc.std <- centroid.std[which(rownames(centroid.std) %in% genes), ]
cc.std <- cc.std[genes2, ]
cc.std
cc.std2 <- cc.std

cc.std <- cc.std2
png(paste0("P307", label, "_Heatmap_sample2.png"), width = 2000, height = 1800, res = 300)
Heatmap(cc.std,
  name = "StdExp",
  row_split = genesgroup[, 2],
  row_title_rot = 0,
  row_names_gp = gpar(fontsize = 8), column_names_gp = gpar(fontsize = 8)
)
dev.off()

library(svglite)
svglite(paste0("P307", label, "_Heatmap_sample2.svg"), width = 7.5, height = 6.75)
Heatmap(cc.std,
  name = "StdExp",
  row_split = genesgroup[, 2],
  row_title_rot = 0,
  row_names_gp = gpar(fontsize = 8), column_names_gp = gpar(fontsize = 8)
)
dev.off()


# Extended Figure 7g. Feature plots of selected marker genes highlight transcriptional changes in amniotic epithelial and mesenchymal states following early patterning.
# early stage except LPM
table(Yearly$type)
Y <- subset(Yearly, type %in% c("SE", "cSKO"))
table(Y$type)
label <- "EarlyNoLPM"

genes <- c("CD44", "MMP9", "BGN", "TESC", "PPP1R1B", "KCNMB1", "ADRB2", "RCSD1", "TBX2", "SYT4", "SLITRK6", "STC1", "MAF", "G0S2", "KRT7", "MFAP5", "NOG", "FST", "SLC15A2", "SLC40A1", "CLIC3", "MUC15", "NPPB", "CA2", "ATF3", "MAFF", "IFI6", "HAND1", "GABRP", "GATA3", "SOX7")
genes[!(genes %in% rownames(Y))]
length(genes) # 31

maxexp <- max(FetchData(Y, vars = genes))
maxexp # [1] 6.071071
nrow <- 5
ncol <- 7
plots <- lapply(genes, function(gene) {
  FeaturePlot(Y, features = gene, pt.size = 0.5, cols = c("grey90", "#DB5545")) + NoLegend() + NoAxes() +
    ggtitle(bquote(italic(.(gene)))) +
    theme(plot.title = element_text(size = 28)) # , pt.size=0.5
})
plotlist <- cowplot::plot_grid(plotlist = plots, ncol = ncol)
png(paste0("P307", label, "_P239.3Annotated", cut, "pct1_Feature.png"), res = 300, height = 850 * nrow, width = 800 * ncol)
plotlist
dev.off()
library(svglite)
svglite(paste0("P307", label, "_P239.3Annotated", cut, "pct1_Feature.svg"), height = 3.1875 * nrow, width = 3 * ncol)
plotlist
dev.off()


plots <- lapply(genes, function(gene) {
  FeaturePlot(Y, features = gene, pt.size = 0.5) + NoLegend() + NoAxes() +
    scale_color_gradientn(colours = c("grey90", "#DB5545"), limits = c(0, maxexp)) +
    ggtitle(bquote(italic(.(gene)))) +
    theme(plot.title = element_text(size = 28)) #
})
plotlist <- cowplot::plot_grid(plotlist = plots, ncol = ncol)
png(paste0("P307", label, "_P239.3Annotated", cut, "pct1_Feature_sameColorScale.png"), res = 300, height = 850 * nrow, width = 800 * ncol)
plotlist
dev.off()

plots <- lapply(genes, function(gene) {
  FeaturePlot(Y, features = gene, pt.size = 0.5) + NoAxes() +
    scale_color_gradientn(colours = c("grey90", "#DB5545"), limits = c(0, maxexp)) +
    ggtitle(bquote(italic(.(gene)))) +
    theme(plot.title = element_text(size = 28)) # , pt.size=0.5
})
plotlist <- patchwork::wrap_plots(plots, ncol = ncol) + plot_layout(guides = "collect") &
  theme(legend.position = "right")
png(paste0("P307", label, "_P239.3Annotated", cut, "pct1_Feature_sameColorScale_legend.png"), res = 300, height = 850 * nrow, width = 850 * ncol)
plotlist
dev.off()


sessionInfo()
R version 4.2.3 (2023-03-15)
Platform: x86_64-conda-linux-gnu (64-bit)
Running under: Rocky Linux 8.10 (Green Obsidian)
Matrix products: default
locale:
 [1] LC_CTYPE=en_US.UTF-8       LC_NUMERIC=C              
 [3] LC_TIME=en_US.UTF-8        LC_COLLATE=en_US.UTF-8    
 [5] LC_MONETARY=en_US.UTF-8    LC_MESSAGES=en_US.UTF-8   
 [7] LC_PAPER=en_US.UTF-8       LC_NAME=C                 
 [9] LC_ADDRESS=C               LC_TELEPHONE=C            
[11] LC_MEASUREMENT=en_US.UTF-8 LC_IDENTIFICATION=C       
attached base packages:
[1] grid      stats     graphics  grDevices utils     datasets  methods  
[8] base     
other attached packages:
[1] ComplexHeatmap_2.14.0 Nebulosa_1.8.0        pbapply_1.7-0        
[4] patchwork_1.1.2       ggplot2_3.4.4         SeuratDisk_0.0.0.9020
[7] SeuratObject_4.1.3    Seurat_4.3.0         

loaded via a namespace (and not attached):
  [1] Rtsne_0.16                  colorspace_2.1-0           
  [3] rjson_0.2.21                deldir_1.0-6               
  [5] ellipsis_0.3.2              ggridges_0.5.4             
  [7] mclust_6.0.0                circlize_0.4.15            
  [9] XVector_0.38.0              GlobalOptions_0.1.2        
 [11] GenomicRanges_1.50.2        clue_0.3-64                
 [13] spatstat.data_3.0-1         leiden_0.4.3               
 [15] listenv_0.9.0               ggrepel_0.9.3              
 [17] bit64_4.0.5                 mvtnorm_1.1-3              
 [19] fansi_1.0.4                 codetools_0.2-19           
 [21] splines_4.2.3               doParallel_1.0.17          
 [23] polyclip_1.10-4             jsonlite_1.8.4             
 [25] ica_1.0-3                   cluster_2.1.4              
 [27] png_0.1-8                   uwot_0.1.14                
 [29] shiny_1.7.4                 sctransform_0.4.1          
 [31] spatstat.sparse_3.0-1       compiler_4.2.3             
 [33] httr_1.4.5                  Matrix_1.5-3               
 [35] fastmap_1.1.1               lazyeval_0.2.2             
 [37] cli_3.6.1                   later_1.3.0                
 [39] htmltools_0.5.5             tools_4.2.3                
 [41] igraph_2.0.3                GenomeInfoDbData_1.2.9     
 [43] gtable_0.3.3                glue_1.6.2                 
 [45] RANN_2.6.1                  reshape2_1.4.4             
 [47] dplyr_1.1.1                 Rcpp_1.0.10                
 [49] Biobase_2.58.0              scattermore_1.2            
 [51] vctrs_0.6.1                 spatstat.explore_3.1-0     
 [53] nlme_3.1-162                progressr_0.13.0           
 [55] iterators_1.0.14            lmtest_0.9-40              
 [57] spatstat.random_3.1-4       stringr_1.5.0              
 [59] globals_0.16.2              mime_0.12                  
 [61] miniUI_0.1.1.1              lifecycle_1.0.3            
 [63] irlba_2.3.5.1               goftest_1.2-3              
 [65] future_1.32.0               zlibbioc_1.44.0            
 [67] MASS_7.3-58.3               zoo_1.8-11                 
 [69] scales_1.2.1                promises_1.2.0.1           
 [71] MatrixGenerics_1.10.0       spatstat.utils_3.0-2       
 [73] parallel_4.2.3              SummarizedExperiment_1.28.0
 [75] RColorBrewer_1.1-3          SingleCellExperiment_1.20.1
 [77] reticulate_1.28             gridExtra_2.3              
 [79] stringi_1.7.12              S4Vectors_0.36.2           
 [81] foreach_1.5.2               BiocGenerics_0.44.0        
 [83] shape_1.4.6                 GenomeInfoDb_1.34.9        
 [85] bitops_1.0-7                rlang_1.1.4                
 [87] pkgconfig_2.0.3             matrixStats_0.63.0         
 [89] pracma_2.4.2                lattice_0.20-45            
 [91] ROCR_1.0-11                 purrr_1.0.1                
 [93] tensor_1.5                  ks_1.14.0                  
 [95] htmlwidgets_1.6.2           cowplot_1.1.1              
 [97] bit_4.0.5                   tidyselect_1.2.0           
 [99] parallelly_1.35.0           RcppAnnoy_0.0.20           
[101] plyr_1.8.8                  magrittr_2.0.3             
[103] R6_2.5.1                    IRanges_2.32.0             
[105] generics_0.1.3              DelayedArray_0.24.0        
[107] pillar_1.9.0                withr_2.5.0                
[109] fitdistrplus_1.1-8          RCurl_1.98-1.12            
[111] survival_3.5-5              abind_1.4-5                
[113] sp_1.6-0                    tibble_3.2.1               
[115] future.apply_1.10.0         crayon_1.5.2               
[117] hdf5r_1.3.8                 KernSmooth_2.23-20         
[119] utf8_1.2.3                  spatstat.geom_3.1-0        
[121] plotly_4.10.1               GetoptLong_1.0.5           
[123] data.table_1.14.8           digest_0.6.31              
[125] xtable_1.8-4                tidyr_1.3.0                
[127] httpuv_1.6.9                stats4_4.2.3               
[129] munsell_0.5.0               viridisLite_0.4.2     